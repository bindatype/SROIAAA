...
Both accept `since` and `until`, but here they bound a ticket's **Created** date, never its open/closed status. That is a different kind of bound than the trap on `fleet.inventory` or `monitoring.problems`: a ticket's creation date cannot change, so narrowing by it only selects which still-open tickets to look at -- it can never make a ticket that is genuinely open disappear from the count the way bounding current state can. Use it for "how many open tickets are older than N days" or "opened this month" -- ask for RT's own exact count with the bound applied, rather than pulling a page of tickets and reading `created` dates off it by eye. `total_matching` and `tickets_by_queue` already reflect the bound once applied; do not additionally filter or count the returned `items` yourself.
...
`breakdown.tickets_by_owner`, when present, is also an exact per-owner count from Request Tracker -- **use it for "group by owner" rather than counting the `items` list.** But unlike `tickets_by_queue`, the set of owners it covers is discovered from the returned page, not from an operator-configured list: when `truncated` is true, an owner with no visible ticket on this page is simply absent from the breakdown even if they own tickets elsewhere in `total_matching`, and a warning says so. In that case report the owner counts as a floor -- "at least 16 for jcreech, more may exist outside this page" -- never as the complete distribution, and never derive a different owner count by reading `items` yourself.
